...
//!
//! Stoat is a Discord-like bot platform. Until its gateway/polling API is
//! confirmed, outbound delivery POSTs `{text,title,url}` to
//! `{STOAT_BASE_URL}/channels/{id}/messages` with `Authorization: Bot <token>`.
//! If that shape differs from the real API, only `post_message` below changes.
//!
...
impl StoatAdapter {
    ...
    async fn post_message(&self, channel: &str, env: &Envelope) -> Result<Vec<String>> {
        ...
        let res = self
            ...
            .send()
            .await
            .map_err(|e| Error::Http(format!("[stoat] post failed: {e}")))?;
...
impl Adapter for StoatAdapter {
    ...
    async fn start(&self) -> Result<()> {
        if self.token.is_none() {
            tracing::warn!("STOAT_TOKEN not set — stoat adapter disabled.");
            return Ok(());
        }
        // Egress path is live; ingress gateway wiring awaits platform docs.
        tracing::warn!("[stoat] live ingress pending API docs; egress delivery active.");
        ...
        let deliver: Deliver = Arc::new(move |env: Envelope| {
            ...
            Box::pin(async move {
                ...
                match res {
                    ...
                    Ok(r) => tracing::error!("[stoat] post rejected: {}", r.status()),
                    Err(e) => tracing::error!("[stoat] post failed: {e}"),
                }
            })
        });
    ...
    async fn process_egress(&self, envelope: &Envelope) -> Result<()> {
        ...
        match outbox {
            ...
            None => tracing::warn!("[stoat] egress with no outbox (not started?)"),
        }
        Ok(())
    }
